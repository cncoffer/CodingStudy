...
BOOL g_fnTinyPlanet(PBIH pBihSrc, PPBIH pResult, int nLat/*=0*/, int nLon/*=0*/, int nZoom/*1*/)
{
		...
		&& sW == src.size().width && sH == src.size().height && nHasDoCalu == TRUE))
	{
		...
		vector<float> vct_map_lat, vct_map_lon;
		vct_map_lat.assign((float*)map_lat.datastart, (float*)map_lat.dataend);
		vct_map_lon.assign((float*)map_lon.datastart, (float*)map_lon.dataend);
		array_view<float, 1> array_lat(vct_map_lat.size(), vct_map_lat);
		array_view<float, 1> array_lon(vct_map_lon.size(), vct_map_lon);

		concurrency::parallel_for_each(
			array_lat.extent,
			[=](index<1> idx) restrict(amp)
		{
			int x = idx[0] % nWidth - nWidth / 2;
			int y = idx[0] / nWidth - nHeight / 2;
			double r = sqrt(x * x + y * y);
			float dElement = 2 * atan(r / 2 / R);
			float temp = PI / 2 + asin(
				cos(dElement) * dSinPhi0
				+ y * sin(dElement)
				* dCosPhi0 / r
				);
			array_lat[idx] = temp * (nHeight - 1) / PI;

			temp = lamda0 + atan2(
				x * sin(dElement),
				r * dCosPhi0 * cos(dElement)
				- y * dSinPhi0 * sin(dElement)
				);
			array_lon[idx] = fmod(temp + PI, 2 * PI) * (nWidth - 1) / (2 * PI);
		});
		array_lat.synchronize();
		array_lon.synchronize();

		remap_y = map_lat;
		remap_x = map_lon;
		std::memcpy(remap_y.data, vct_map_lat.data(), vct_map_lat.size()*sizeof(float));
		std::memcpy(remap_x.data, vct_map_lon.data(), vct_map_lon.size()*sizeof(float));
